...
func TestCreateToolFunction_Multipart_NestedBinaryField_FileObjectValue_FilenamePreserved(
	...
	require.NoError(t, err)
	require.Equal(t, "invoice.pdf", capturedFilename)
}
...
func TestCreateToolFunction_Multipart_ArrayOfObjects_OneOfDiscriminator_UsesMatchingBranch(
	...
	t.Setenv("TEST", "true")
	// report.report_form_inputs は type プロパティを discriminator とする oneOf の配列。値の type
	// に対応するブランチだけが使われ、別ブランチにしか無いフィールド名（value）が誤って使われたり、
	// isFile 判定を持つブランチ（file）が対応しないブランチの値に適用されたりしないことを確認する。
	fileInputSchema := &openapi3.Schema{
		Type: &openapi3.Types{"object"},
		Properties: openapi3.Schemas{
			"type": &openapi3.SchemaRef{
				Value: &openapi3.Schema{
					Type: &openapi3.Types{"string"},
					Enum: []any{"FormInputInvoiceFileField"},
				},
			},
			"id": &openapi3.SchemaRef{
				Value: &openapi3.Schema{Type: &openapi3.Types{"string"}},
	...
	textInputSchema := &openapi3.Schema{
		Type: &openapi3.Types{"object"},
		Properties: openapi3.Schemas{
			"type": &openapi3.SchemaRef{
				Value: &openapi3.Schema{
					Type: &openapi3.Types{"string"},
					Enum: []any{"FormInputTextField"},
				},
			},
			"id": &openapi3.SchemaRef{
				Value: &openapi3.Schema{Type: &openapi3.Types{"string"}},
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		require.NoError(t, r.ParseMultipartForm(10<<20))
		capturedValues = r.MultipartForm.Value
		f, _, err := r.FormFile("report[report_form_inputs][0][file]")
		require.NoError(t, err)
		defer f.Close() //nolint: errcheck
			...
			"report": &openapi3.SchemaRef{
				Value: &openapi3.Schema{
					...
					Properties: openapi3.Schemas{
						...
						"report_form_inputs": &openapi3.SchemaRef{
							Value: &openapi3.Schema{
								Type:  &openapi3.Types{"array"},
	...
	_, _, err := fn(context.Background(), map[string]any{
		"report": map[string]any{
			"title": "Q1 report",
			"report_form_inputs": []any{
				map[string]any{
					"type": "FormInputInvoiceFileField",
					"id":   "field-1",
	...
	require.Equal(t, []string{"Q1 report"}, capturedValues["report[title]"])
	require.Equal(t, []string{"field-1"}, capturedValues["report[report_form_inputs][0][id]"])
	require.Equal(
		t,
		[]string{"FormInputInvoiceFileField"},
		capturedValues["report[report_form_inputs][0][type]"],
	)
	require.Equal(t, []string{"field-2"}, capturedValues["report[report_form_inputs][1][id]"])
	require.Equal(t, []string{"hello"}, capturedValues["report[report_form_inputs][1][value]"])
	// TypeB (index 1) にしか無い "value" が TypeA (index 0) のフィールドに誤って使われていない
	require.NotContains(t, capturedValues, "report[report_form_inputs][0][value]")
