...
| # | 論点 | 推奨 | 状態 |
| --- | --- | --- | --- |
| 1 | WS チャネル満杯時に MCP フレームを drop する現挙動 | **現状維持**。ブロッキングは単一 binding の詰まりが同一接続の全 binding を止める。呼び出し側は callTimeout で失敗が返る。将来、per-binding のバックプレッシャが必要になったら Phase 3 の中継設計と合わせて再検討 | 承認済み（2026-08-22、現状維持で確定） |
| 2 | jwt プロファイルのフェーズ区分（正本文書が Phase 1 に読める） | **本書で Phase 2a と明確化**（正本文書の識別セクションに「実装は Phase 2a」の注記を追加する） | 本書で解消 |
| 3 | static モード時の bind アドレス（全インターフェース） | **警告もバリデーションも追加しない**。コンテナ/k8s では 0.0.0.0 が正常であり画一的警告は誤検知になる。「static はローカル専用。公開ネットワークに晒さない」を README / 正本文書の static 節に明記する対応のみ | 承認済み（2026-08-22、正本文書 static 節・拡張 README に明記済み） |
...
| 5 | `edge.pairing.type` の既定値変更（static→remote）の移行影響 | 既存の reverse 構成で `pairing.type` を明示していない場合、`identity` プロファイル未設定だと起動時バリデーションが失敗する。回避策は `pairing.type: static` を明示指定。未リリース機能で既存利用者がいないため、利用者向けドキュメントへの移行注記は不要と判断 | 2a で確定、本書に記載 |
| 6 | `/edge/pair` の IP レート制限が信頼するプロキシの範囲 | 既定は RFC1918 プライベート IP のみ（インターネット越しに送信元 IP を偽装できないため、デプロイ形態を問わず安全）。Cloudflare 配下で運用する場合は `edge.trustCloudflare: true` を明示指定（Cloudflare 公開 IP レンジのスナップショットをコードに保持し、依存ライブラリ `netinternet/remoteaddr` のバージョンアップ時にテストで整合性を検知——レンジが Cloudflare の手を離れてもコード側に残り続ける「stale allowlist」を防ぐ）。ALB/Ingress 等 RFC1918 外の CIDR を使うプロキシは `edge.trustedForwarders` で追加指定 | 2a のスコープに編入 |
| 7 | RFC1918 全体を既定で信頼するのは広すぎるのでは（CodeRabbit 再指摘、review 4999886095） | **対応不要（現状維持）**。同一ネットワーク内の侵害ホストが `X-Forwarded-For` を偽装できる、という指摘自体は成立するが、その脅威モデルへの対処（信頼するプロキシ CIDR をネットワークセグメントごとに限定する等）はネットワーク設計・ゾーニングの責務であり、ゲートウェイである Manifold 側で作り込む運用上の必要性は薄いと判断。現行の「RFC1918 は既定で信頼、Cloudflare/その他は明示オプトイン」を維持する | 却下（2026-08-22） |
